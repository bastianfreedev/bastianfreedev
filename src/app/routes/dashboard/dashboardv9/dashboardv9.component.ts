...
@Component({
  selector: "app-dashboardv9",
  templateUrl: "./dashboardv9.component.html",
  styleUrls: ["./dashboardv9.component.scss"],
  encapsulation: ViewEncapsulation.None,
})
export class Dashboardv9Component implements OnInit {
  ...
  columns = [{ prop: "id_cargo" }, { name: "Codigo" }];
  columnsSort = [{ prop: "id_cargo" }];
  ...
  constructor(
    ...
    this.enterpriseApi.getCharges().then((res) => {
      console.log(res);
    });
  ...
  onSubmit() {
    ...
    this.enterpriseApi
      .postCharges(this.profileForm.value)
  ...
  onSubmitUpdate() {
    ...
    this.enterpriseApi
      .putCharges(this.profileForm.value)
      .then((res) => {
        this.refresh();
  ...
  fetch(cb) {
    this.enterpriseApi.getCharges().then((res: any) => {
      cb(res.data);
      //debugger;
    });
  ...
  updateFilter(event) {
    ...
    const temp = this.temp.filter(function (d) {
      return d.nombre_cargo.toLowerCase().indexOf(val) !== -1 || !val;
    });
  ...
  updateFilterCodigo(event) {
    const val = event.target.value.toLowerCase();

    const temp = this.temp.filter(function (d) {
      return d.id_cargo.toLowerCase().indexOf(val) !== -1 || !val;
    });
